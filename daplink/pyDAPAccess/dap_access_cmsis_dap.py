...
from .cmsis_dap_core import COMMAND_ID, DAP_TRANSFER_OK, DAP_TRANSFER_FAULT, DAP_TRANSFER_WAIT
...
class _Transfer(object):
    ...
    def __init__(self, daplink, dap_index, transfer_count, transfer_request, transfer_data):
    ...
    def add_response(self, data):
        ...
        for i in range(0, self._size_bytes, 4):
            word = ((data[0 + i] << 0) | (data[1 + i] << 8) | (data[2 + i] << 16) | (data[3 + i] << 24))
            result.append(word)
        self._result = result
...
class DAPAccessCMSISDAP(DAPAccessIntf):
    ...
    def get_connected_devices():
        ...
        """
        all_daplinks = []
        all_interfaces = INTERFACE[usb_backend].getAllConnectedInterface()
        for interface in all_interfaces:
            try:
                unique_id = interface.getSerialNumber()
                new_daplink = DAPAccessCMSISDAP(unique_id, interface.product_name)
    ...
    # ------------------------------------------- #
    def __init__(self, unique_id, product_name=''):
    ...
    def _init_deferred_buffers(self):
        ...
        """
        # List of transfers that have been started, but not completed (started by write_reg, read_reg,
    ...
    def _send_packet(self):
        ...
        This function guarentees that the number of packets that are stored in daplink's buffer (the number of
        packets written but not read) does not exceed the number supported by the given device.
        """
        cmd = self._crnt_cmd
        if cmd.get_empty():
            return
    ...
    def _write(self, dap_index, transfer_count, transfer_request, transfer_data):
        """
        ...
        transfer = None
        if transfer_request & READ:
            transfer = _Transfer(self, dap_index, transfer_count, transfer_request, transfer_data)
